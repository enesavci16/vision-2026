...
class Track:
    ...
    def __init__(self, track_id: int, initial_detection: np.ndarray):
        ...
        self.kf.F = np.array([[1., 0., 1., 0.],  # x = x + v_x
                              [0., 1., 0., 1.],  # y = y + v_y
                              [0., 0., 1., 0.],  # v_x = v_x
                              [0., 0., 0., 1.]]) # v_y = v_y
        ...
        self.kf.x = np.array([[initial_detection[0]], 
                              [initial_detection[1]], 
                              [0.], 
                              [0.]])
...
class MultiObjectTracker:
    ...
    def _compute_cost_matrix(self, track_positions: np.ndarray, detections: np.ndarray) -> np.ndarray:
    ...
    def update(self, detections: np.ndarray) -> List[Tuple[int, np.ndarray]]:
        ...
        if len(detections) == 0:
            ...
            return [(t.track_id, t.position) for t in self.tracks if t.time_since_update < 1]
