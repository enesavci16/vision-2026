...
def run_1d_simulation():
    ...
    kf.Q = np.array([[0.1, 0.0], 
                     [0.0, 0.1]]) # Process noise (Small accelerations)
    ...
    plt.plot(true_positions, label='True Position (Reality)', linestyle='--', color='black')
    plt.scatter(range(50), yolo_measurements, label='YOLO Detections (Noisy)', color='red', marker='x')
    plt.plot(kalman_estimates, label='Kalman Filter Estimate', color='blue', linewidth=2)
