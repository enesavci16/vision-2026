...
class LaneCounter:
    ...
    def __init__(self, line_pt1: Tuple[float, float], line_pt2: Tuple[float, float], history_size: int = 5):
    ...
    def _intersects(self, C_t_minus_1: np.ndarray, C_t: np.ndarray) -> bool:
        ...
        straddle_1 = self._ccw(self.A, C_t_minus_1, C_t) != self._ccw(self.B, C_t_minus_1, C_t)
        
        # Condition 2: Does the car's path straddle line A-B?
        straddle_2 = self._ccw(self.A, self.B, C_t_minus_1) != self._ccw(self.A, self.B, C_t)
